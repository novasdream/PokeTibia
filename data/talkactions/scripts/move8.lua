...
setConditionFormula(thunderwavecondition, -0.75, -0.75, -0.75, -0.75)
 ...
 snorlax = createCombatArea(pokeSpell.snorlax)
 skullb = createCombatArea(pokeSpell.skullb)
 twn = createCombatArea(pokeSpell.twn)
 tws = createCombatArea(pokeSpell.tws)
 tww = createCombatArea(pokeSpell.tww)
 twe = createCombatArea(pokeSpell.twe)
 ...
 stomp = createCombatArea(pokeSpell.stomp)
 ...
 pump1 = createCombatArea(pokeSpell.pump1)
 pump2 = createCombatArea(pokeSpell.pump2)
 tor1 = createCombatArea(pokeSpell.tor1)
 tor2 = createCombatArea(pokeSpell.tor2)
 tor3 = createCombatArea(pokeSpell.tor3)
 earthquake = createCombatArea(pokeSpell.earthquake)
...
if getTilePzInfo(getPlayerPosition(cid)) then
	...
	if not pokemon.move8 then
	doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_BLUE, "Your pokemon doesn\'t recognize this move.")
	return 0
	end
	if getPlayerLevel(cid) < pokemon.move8.minLv then
...
if pokemon.move8.target ~= "no" then
	if not isMonster(getCreatureTarget(cid)) then
	doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_BLUE, "You don\'t have any targets.")
	return 0
	end

	if getDistanceBetween(getThingPos(getCreatureSummons(cid)[1]), getThingPos(getCreatureTarget(cid))) > pokemon.move8.dista then
	...
	if pokemon.move8.type == "flying" then
		elementa = FLYDAMAGE
	elseif pokemon.move8.type == "ground" then
		elementa = GROUNDDAMAGE
	elseif pokemon.move8.type == "electric" then
		elementa = ELECTRICDAMAGE
	elseif pokemon.move8.type == "ghost" then
		elementa = GHOSTDAMAGE
	elseif pokemon.move8.type == "normal" then
		elementa = NORMALDAMAGE
	elseif pokemon.move8.type == "fighting" then
		elementa = FIGHTINGDAMAGE
	elseif pokemon.move8.type == "poison" then
		elementa = POISONDAMAGE
	elseif pokemon.move8.type == "rock" then
		elementa = ROCKDAMAGE
	elseif pokemon.move8.type == "bug" then
		elementa = BUGDAMAGE
	elseif pokemon.move8.type == "fire" then
		elementa = FIREDAMAGE
	elseif pokemon.move8.type == "water" then
		elementa = WATERDAMAGE
	elseif pokemon.move8.type == "grass" then
		elementa = GRASSDAMAGE
	elseif pokemon.move8.type == "psychic" then
		elementa = PSYCHICDAMAGE
	elseif pokemon.move8.type == "ice" then
		elementa = ICEDAMAGE
	elseif pokemon.move8.type == "dragon" then
		elementa = DRAGONDAMAGE
	end

	local element = elementa
	local num1 = (pokemon.move8.base1)
	local num2 = (pokemon.move8.base2)
	local num3 = (pokemon.move8.bonus)
...
if(pokename:find("\+(%d+)"))then 
	num1 = ((pokemon.move8.base1)*(boost*(boostrate*2)))
	num2 = ((pokemon.move8.base2)*boost)
	num3 = (pokemon.move8.bonus)
else
	num1 = (pokemon.move8.base1)
	num2 = (pokemon.move8.base2)
	num3 = (pokemon.move8.bonus)
end
	...
	if pokemon.move8.spell == "Healeara" then
		doCreatureSay(getCreatureSummons(cid)[1], ""..string.upper(pokemon.move8.spell).."!", TALKTYPE_MONSTER)
	...
	elseif pokemon.move8.spell == "Rock Slide" then
		...
		addEvent(bolt, 200, {cid = cid, boltalvo = boltalvo})
	        elseif pokemon.move8.spell == "Gust" then
		doCreatureSay(getCreatureSummons(cid)[1], ""..string.upper(pokemon.move8.spell).."!", TALKTYPE_MONSTER)
		doCreatureSay(cid, ""..getCreatureName(getCreatureSummons(cid)[1])..", use "..pokemon.move8.spell.."!", TALKTYPE_SAY)
		exhaustion.set(cid, pokemon.move8.ex, pokemon.move8.cd)
