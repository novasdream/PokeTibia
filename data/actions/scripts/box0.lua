...
r1 = math.random(1,4)
if getPlayerLevel(cid) >= 0 and getPlayerFreeCap(cid) ~= 0 then
if doPlayerRemoveItem(cid,188,1) == TRUE then
if r1 == 1 then
...
elseif r1 == 4 then
doPlayerSendTextMessage(cid,22,"Você ganhou um Sunkern.")
local health = 350 -- Health do poke
local maxhealth = 350 -- Maximo de health do poke
local description = "Contains a Sunkern."
local poke1 = "This is Sunkern's pokeball. HP = ["..health.."/"..maxhealth.."]"
item = doCreateItemEx(2219)
doItemSetAttribute(item, "poke", poke1)
doItemSetAttribute(item, "nome", "Sunkern")
doItemSetAttribute(item, "description", description)
doPlayerAddItemEx(cid, item, true)
doTransformItem(item, 2222)
doSendMagicEffect(getCreaturePosition(cid), 28)
